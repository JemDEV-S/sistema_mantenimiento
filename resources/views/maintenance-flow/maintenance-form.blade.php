...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
        <h1 class="h2">Registro de Mantenimiento</h1>
        ...
        <div class="row">
            ...
            <div class="col-md-5">
                ...
                <div class="d-flex gap-3 flex-wrap">
                    ...
                    <div><strong>S/N:</strong> {{ $asset->serial }}</div>
                    <div><strong>Marca/Modelo:</strong> {{ $asset->brand }} {{ $asset->model }}</div>
                </div>
            </div>
            <div class="col-md-4">
                <div><strong>Departamento:</strong> {{ $asset->department->name }}</div>
                <div><strong>Código Patrimonial:</strong> {{ $asset->patrimony_code ?? 'No asignado' }}</div>
            ...
            <div class="col-md-2 text-end">
                <a href="{{ route('assets.show', $asset->id) }}" class="btn btn-sm btn-outline-info" target="_blank">
    ...
    <div class="form-wizard">
        ...
        <form id="maintenance-form" action="{{ route('maintenance-flow.store-maintenance', $asset->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            <div class="wizard-content">
                <!-- Paso 1: Información Básica -->
                <div class="tab-pane active" id="step1">
                    <h4 class="mb-4">Información Básica del Mantenimiento</h4>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-12">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <div class="card maintenance-type-card" data-type="preventive">
                                        <div class="card-body text-center">
                                            <i class="fas fa-calendar-check fa-3x text-success mb-3"></i>
                                            <h5>Preventivo</h5>
                                            <p class="text-muted small">Mantenimiento programado para prevenir fallos y extender la vida útil del equipo.</p>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="maintenance_type" id="type_preventive" value="preventive" required>
                                                <label class="form-check-label" for="type_preventive">Seleccionar</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="card maintenance-type-card" data-type="corrective">
                                        <div class="card-body text-center">
                                            <i class="fas fa-tools fa-3x text-danger mb-3"></i>
                                            <h5>Correctivo</h5>
                                            <p class="text-muted small">Mantenimiento para reparar fallos o averías ya existentes en el equipo.</p>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="maintenance_type" id="type_corrective" value="corrective" required>
                                                <label class="form-check-label" for="type_corrective">Seleccionar</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="card maintenance-type-card" data-type="predictive">
                                        <div class="card-body text-center">
                                            <i class="fas fa-chart-line fa-3x text-info mb-3"></i>
                                            <h5>Predictivo</h5>
                                            <p class="text-muted small">Mantenimiento basado en la condición del equipo para anticipar posibles fallos.</p>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="maintenance_type" id="type_predictive" value="predictive" required>
                                                <label class="form-check-label" for="type_predictive">Seleccionar</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control" id="estimated_time" name="estimated_time" min="5" placeholder="Ingrese tiempo estimado">
                            <div class="form-text">Aproximado del tiempo que tomará realizar este mantenimiento</div>
                        </div>
                    </div>
                    
                    <!-- Timer para controlar tiempo de trabajo -->
                    <div class="timer-container">
                        <div>
                            <label class="form-label mb-0">Cronómetro de Trabajo</label>
                            <div class="timer-display" id="timer-display">00:00:00</div>
                        </div>
                        <div>
                            <button type="button" class="btn btn-outline-primary" id="timer-start">
                                <i class="fas fa-play"></i> Iniciar
                            </button>
                            <button type="button" class="btn btn-outline-secondary" id="timer-pause" disabled>
                                <i class="fas fa-pause"></i> Pausar
                            </button>
                            <button type="button" class="btn btn-outline-danger" id="timer-reset" disabled>
                                <i class="fas fa-redo"></i> Reiniciar
                            </button>
                        </div>
                    </div>
                    
                    <!-- Si hay mantenimientos previos, mostrarlos -->
                    @if(count($previousMaintenances) > 0)
                    <div class="card mt-4">
                        <div class="card-header bg-light">
                            <i class="fas fa-history me-1"></i> Historial de Mantenimientos Recientes
                        </div>
                            ...
                            <div class="list-group list-group-flush">
                                ...
                                <div class="list-group-item">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            <span class="badge @if($prev->maintenance_type == 'preventive') bg-success @elseif($prev->maintenance_type == 'corrective') bg-danger @else bg-info @endif">
                                                {{ ucfirst($prev->maintenance_type) }}
                                            </span>
                                            <small class="text-muted">Fecha: {{ $prev->end_date->format('d/m/Y') }}</small>
                                        </div>
                                        <button type="button" class="btn btn-sm btn-outline-secondary view-previous" data-id="{{ $prev->id }}">
                                            <i class="fas fa-eye"></i> Ver detalles
                                        </button>
                                    </div>
                                    <div class="previous-details" id="previous-{{ $prev->id }}" style="display: none;">
                                        <div class="mt-2 border-top pt-2">
                                            <p><strong>Diagnóstico:</strong> {{ $prev->diagnosis }}</p>
                                            <p><strong>Procedimiento:</strong> {{ $prev->procedure }}</p>
                                            <p class="mb-0">
                                                <a href="{{ route('maintenances.show', $prev->id) }}" target="_blank" class="btn btn-sm btn-link">
                                                    <i class="fas fa-external-link-alt"></i> Ver mantenimiento completo
                                                </a>
                                            </p>
                                        </div>
                                    </div>
                ...
                <div class="tab-pane" id="step2">
                    <h4 class="mb-4">Diagnóstico del Problema</h4>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="list-group common-issues">
                                        @if($asset->type == 'Desktop' || $asset->type == 'Laptop')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control summernote" id="diagnosis" name="diagnosis" required></textarea>
                ...
                <div class="tab-pane" id="step3">
                    <h4 class="mb-4">Procedimiento de Mantenimiento</h4>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control summernote" id="procedure" name="procedure" required></textarea>
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="materials_used" name="materials_used" rows="4"></textarea>
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="recommendations" name="recommendations" rows="4"></textarea>
                                ...
                                <div class="card-body">
                                    <ul class="list-group list-group-flush" id="maintenance-summary">
                                        ...
                                        <li class="list-group-item d-flex justify-content-between">
                                            ...
                                            <span id="summary-worked-time">00:00:00</span>
                                        </li>
                                    </ul>
                                </div>
                                <div class="card-footer bg-light text-center">
                                    <p class="mb-0">
                                        <small class="text-muted">El mantenimiento quedará registrado por: {{ Auth::user()->name }}</small>
                                    </p>
                                </div>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Paso 1: Configuración de tipos de mantenimiento
        const maintenanceTypeCards = document.querySelectorAll('.maintenance-type-card');
        const maintenanceTypeRadios = document.querySelectorAll('input[name="maintenance_type"]');
        
        maintenanceTypeCards.forEach(card => {
            card.addEventListener('click', function() {
                const type = this.dataset.type;
                // Limpiar selección anterior
                maintenanceTypeCards.forEach(c => c.classList.remove('selected'));
                maintenanceTypeRadios.forEach(r => r.checked = false);
        ...
        function updateTimerDisplay() {
            ...
            document.getElementById('summary-worked-time').textContent = formattedTime;
        }
        
        // Botones para ver mantenimientos previos
        const viewPreviousButtons = document.querySelectorAll('.view-previous');
        viewPreviousButtons.forEach(button => {
            button.addEventListener('click', function() {
                const id = this.dataset.id;
                const details = document.getElementById(`previous-${id}`);
                
                if (details.style.display === 'none') {
                    details.style.display = 'block';
                    this.innerHTML = '<i class="fas fa-eye-slash"></i> Ocultar detalles';
                } else {
                    details.style.display = 'none';
                    this.innerHTML = '<i class="fas fa-eye"></i> Ver detalles';
                }
            });
        });
                        ...
                        mediaRecorder.addEventListener('stop', () => {
                            ...
                            setTimeout(() => {
                                ...
                                setTimeout(() => {
                                    ...
                                    $('#diagnosis').summernote('code', diagnosisEditor + '<p>[Texto transcrito del audio] El equipo presenta problemas de rendimiento y muestra mensajes de error al iniciar aplicaciones. Se observa sobrecalentamiento en la zona del procesador.</p>');
        ...
        nextBtn.addEventListener('click', function() {
            ...
            if (currentStep === 1) {
                // Verificar selección de tipo
                if (!document.querySelector('input[name="maintenance_type"]:checked')) {
